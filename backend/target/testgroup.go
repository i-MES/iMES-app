...
var laststatus map[string][]TestItemStatus

func GetTestItemStatuses(teid string) []TestItemStatus {
	return laststatus[teid]
}
...
func (tg *TestGroup) Run(ctx *context.Context, teid string) {
	if laststatus == nil {
		laststatus = make(map[string][]TestItemStatus, 0)
	}
	...
	for _, tc := range tg.TestClasses {
		tc.Run(ctx, func(ename string, tiid string, msg string) {
			...
			// 在后端缓存所有 status，以备前端随时查取
			if laststatus[teid] == nil {
				laststatus[teid] = []TestItemStatus{newtis}
			} else {
				found := false
				for idx, tis := range laststatus[teid] {
					if (tis.TestGroupId == tg.Id) && (tis.TestClassId == tc.Id) && (tis.TestItemId == tiid) {
						laststatus[teid][idx] = newtis
						found = true
						break
					}
				}
				if !found {
					laststatus[teid] = append(laststatus[teid], newtis)
				}
			}
		})
	}
