    ...
    <v-card class="elevation-24 mx-0 v-card--tilog" color="grey-lighten-4"
      ...
      <v-toolbar height="48">
        ...
        <v-btn icon="mdi-download-outline"
          @click="((logHeight - 50) < 48) ? 48 : logHeight -= 50">
        </v-btn>
      ...
      <div class="log-table" @scroll.passive="onScroll($event)">
        <v-table density="compact" :height="`${logHeight - 48}px`">
          ...
          <tbody>
            <tr v-for="(log, idx) in store.testitemsLogs" :key="log.timestamp">
              <td>{{ idx + 1 }}</td>
              <td>{{
                  DateTime.fromSeconds(log.timestamp).toFormat('yyyy-MM-dd HH:MM:ss')
              }}</td>
              <td>{{ log.message }}</td>
            </tr>
          </tbody>
